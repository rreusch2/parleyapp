-- Add betting results column to track over/under performance against sportsbook lines
ALTER TABLE player_game_stats 
ADD COLUMN betting_results jsonb DEFAULT '{}'::jsonb;

-- Add an index for efficient querying
CREATE INDEX idx_player_game_stats_betting_results ON player_game_stats USING GIN (betting_results);

-- Add a comment to explain the structure
COMMENT ON COLUMN player_game_stats.betting_results IS 'JSON object storing betting line results, e.g. {"hits": {"line": 1.5, "result": "under", "sportsbook": "draftkings"}}';
