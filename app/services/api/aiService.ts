...
class AIService {
  ...
  async getTodaysPicks(userId?: string, userTier?: string): Promise<AIPrediction[]> {
    try {
      ...
      if (data.success && data.total_picks > 0) {
        ...
        console.log('ðŸ”§ DEBUG - Raw pick sample BEFORE transformation:', {
          pick: rawPicks[0],
        ...
        console.log(`ðŸ“š Loaded ${allPicks.length} AI picks (${data.breakdown?.team_picks || 0} team + ${data.breakdown?.player_props_picks || 0} props)`);
        console.log('ðŸ”§ DEBUG - Transformed picks sample:', allPicks.slice(0, 2));
        return allPicks;
      }
      
      // If no predictions found, return empty array
      console.log('ðŸ“š No predictions found in database');
      return [];
    } catch (error) {
  ...
  async generateNewPicks(userId?: string): Promise<AIPrediction[]> {
    try {
      console.log('ðŸ¤– Generating new AI picks via DeepSeek orchestrator...');
      const currentUserId = userId || await this.getCurrentUserId();
      ...
      console.log('âœ… New AI picks generated successfully');
  ...
  async getStarterPicks(userId?: string): Promise<AIPrediction[]> {
    try {
      const currentUserId = userId || await this.getCurrentUserId();
      console.log('ðŸŽ Getting instant starter picks for new user...');
      
      const data = await this.makeRequest(`${BACKEND_URL}/api/ai/starter-picks?userId=${currentUserId}`);
      
      if (data.success && data.predictions) {
        console.log(`âœ… Received ${data.predictions.length} starter picks instantly!`);
        console.log(`ðŸ“¦ Source: ${data.metadata?.picksSource}`);
        return data.predictions;
      }
  ...
  async generateFirstPicks(userId?: string): Promise<AIPrediction[]> {
    try {
      const currentUserId = userId || await this.getCurrentUserId();
      console.log('ðŸŽ Generating first picks for new user...');
      ...
      console.log('âœ… First picks generated for new user');
      return data.predictions || [];
    } catch (error) {
