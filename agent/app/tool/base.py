...
class ToolResult(BaseModel):
    ...
    def __bool__(self):
        # Pydantic v2 stores fields in model_fields; fall back to known names
        try:
            field_names = list(getattr(self, 'model_fields').keys())  # type: ignore[attr-defined]
        except Exception:
            field_names = ['output', 'error', 'base64_image', 'system']
        return any(getattr(self, name, None) for name in field_names)
...
class BaseTool(ABC, BaseModel):
    ...
    # Pydantic v2 configuration: allow arbitrary runtime attributes on tools
    # (e.g., sessions, clients) and arbitrary types like HTTP/Supabase clients.
