#!/usr/bin/env python3
"""
College Football Historical Player Stats Ingestion System
========================================================
Fetches CFB players' last 10 games across multiple seasons from SportsData.io
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/home/reid/Desktop/parleyapp/logs/cfb-historical-stats.log'),
...
def get_historical_seasons() -> List[int]:
    """Get list of historical seasons to process"""
    current_year = datetime.now().year
    current_month = datetime.now().month
    
    # Include last 3 completed seasons
    if current_month >= 8:  # CFB season has started
        return [current_year - 2, current_year - 1, current_year]
    else:  # Off-season
        return [current_year - 3, current_year - 2, current_year - 1]

def fetch_cfb_teams() -> List[Dict]:
    """Fetch all CFB teams"""
    url = f"{SPORTSDATA_BASE_URL}/scores/json/teams?key={SPORTSDATA_API_KEY}"
    
    logger.info("Fetching CFB teams")
    data = rate_limited_request(url)
    
    if data:
        logger.info(f"Retrieved {len(data)} CFB teams")
        return data
    else:
        logger.error("Failed to fetch CFB teams")
        return []

def fetch_cfb_team_players(team_key: str) -> List[Dict]:
    """Fetch players for a specific CFB team"""
    ...
    if data:
        logger.info(f"Retrieved {len(data)} players for team {team_key}")
        return data
    else:
...
def fetch_player_game_logs_for_season(player_id: int, season: int) -> List[Dict]:
    ...
    url = f"{SPORTSDATA_BASE_URL}/stats/json/PlayerGameStatsByPlayer/{season}/{player_id}?key={SPORTSDATA_API_KEY}"
...
def get_last_n_games_for_player(player_id: int, seasons: List[int], target_games: int = 10) -> List[Dict]:
    """Get last N games for a player across multiple seasons"""
    all_games = []
    ...
    for season in reversed(seasons):
        ...
        for game in game_logs:
            ...
            if game.get('DateTime'):
                try:
                    game_datetime = datetime.fromisoformat(game['DateTime'].replace('Z', '+00:00'))
                    game['SortDate'] = game_datetime
                except:
...
def create_or_update_cfb_player(player_data: Dict, team_info: Dict) -> Optional[str]:
    """Create or update CFB player in database"""
    try:
        player_name = f"{player_data.get('FirstName', '')} {player_data.get('LastName', '')}".strip()
        team_name = team_info.get('School', team_info.get('Name', ''))
        ...
        existing_player = supabase.table('players').select('id').eq('name', player_name).eq('sport', 'College Football').eq('team', team_name).execute()
        ...
        new_player = {
            ...
            'player_key': f"cfb_{player_name.lower().replace(' ', '_')}_{team_name.lower().replace(' ', '_')}",
            'metadata': {
        ...
        if result.data:
            logger.info(f"Created CFB player: {player_name} ({team_name})")
...
def map_cfb_stats_to_standard_format(game_stats: Dict) -> Dict[str, Any]:
    """Map CFB game stats to standardized format"""
...
def process_team_players(team: Dict, target_games: int = 10) -> int:
    ...
    team_name = team.get('School', team.get('Name', ''))
    ...
    # Get team players
    players = fetch_cfb_team_players(team_key)
    ...
    for player in players:
        ...
        player_id = create_or_update_cfb_player(player, team)
        ...
        if recent_games:
            for game_stats in recent_games:
                ...
                mapped_stats = map_cfb_stats_to_standard_format(game_stats)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Populate CFB player historical stats')
    ...
    logger.info("Starting CFB historical player stats ingestion")
    
    try:
        # Get all CFB teams
        teams = fetch_cfb_teams()
        
        if not teams:
            logger.error("No CFB teams found")
            return
        ...
        logger.info(f"CFB historical stats ingestion completed. Total processed: {total_processed}")
