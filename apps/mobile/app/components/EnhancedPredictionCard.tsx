...
export default function EnhancedPredictionCard({ prediction, index, onAnalyze, welcomeBonusActive = false }: Props) {
  ...
  // Rich metadata from backend generator
  const meta: any = (prediction as any).metadata || {};
  const headshotUri: string | undefined = meta.player_headshot_url;
  const leagueLogoUri: string | undefined = meta.league_logo_url;
  const isAlt: boolean = !!meta.is_alt;
  const playerName: string = meta.player_name || String(prediction.pick || '').split(' ').slice(0, 2).join(' ');
  const initials: string = playerName ? playerName.split(' ').map((p: string) => p[0]).filter(Boolean).slice(0,2).join('').toUpperCase() : '?';
  ...
  const getSportIcon = (sport?: string) => {
    if (!sport) return 'ðŸŸï¸';
    switch (sport.toLowerCase()) {
      case 'nba': return 'ðŸ€';
      case 'nfl': return 'ðŸˆ';
      case 'mlb': return 'âš¾';
      case 'nhl': return 'ðŸ’';
      default: return 'ðŸŸï¸';
    }
  };
        ...
        >
          ...
          <View style={styles.header}>
            ...
            <View style={styles.avatarContainer}>
              {headshotUri ? (
                <OptimizedImage source={{ uri: headshotUri }} style={styles.avatarImage} />
              ) : (
            ...
            <View style={styles.matchInfo}>
              <View style={styles.sportBadge}>
                {leagueLogoUri ? (
                  <OptimizedImage source={{ uri: leagueLogoUri }} style={styles.leagueLogo} />
                ) : null}                <Text style={styles.sportIcon}>{getSportIcon(prediction.sport)}</Text>
                <Text style={styles.sportText}>{prediction.sport || 'Unknown'}</Text>
              </View>
              {/* Display the matchup â€“ fall back to `match_teams` if `match` is missing (e.g., Pro picks) */}
            ...
            <View style={styles.pickSection}>
              ...
              <Text style={[styles.pickValue, isElite && { color: theme.accentPrimary }]}>{prediction.pick || 'Loading...'}</Text>
                ...
                {((prediction as any).metadata?.bookmaker_logo_url) ? (
                  <OptimizedImage source={{ uri: (prediction as any).metadata.bookmaker_logo_url }} style={styles.bookLogo} />
                ) : null}
                <Text style={styles.oddsText}>Odds: {formatOdds(prediction.odds)}</Text>
                {((prediction as any).metadata?.is_alt) ? (
                  <Text style={styles.altPill}>ALT</Text>
                ) : null}
              </View>
            </View>
            ...
            <View style={[styles.premiumSection, welcomeBonusActive && !isPro && styles.bonusSection]}> 
              <View style={styles.premiumBadge}>
                ...
                {welcomeBonusActive && !isPro && (
                  <Text style={styles.bonusIndicator}>ðŸŽ</Text>
                )}
              </View>
            ...
            <View style={styles.modalContent}>
              ...
              >
                ...
                <ScrollView style={styles.analysisContent}>
                  ...
                  <View style={styles.analysisSection}>
                    <Text style={styles.factorTitle}>ðŸ§  AI Reasoning</Text>
                    <Text style={styles.factorText}>{prediction.reasoning}</Text>
                  </View>

                  {/* Analysis Factors */}
                  <View style={styles.analysisSection}>
                    <Text style={styles.factorTitle}>ðŸ“Š Predictive Analytics</Text>
                    <Text style={styles.factorText}>{advancedAnalysis.factors.predictiveAnalytics}</Text>
                  </View>

                  <View style={styles.analysisSection}>
                    <Text style={styles.factorTitle}>ðŸ“° Recent Intelligence</Text>
                    <Text style={styles.factorText}>{advancedAnalysis.factors.recentNews}</Text>
                  </View>

                  <View style={styles.analysisSection}>
                    <Text style={styles.factorTitle}>ðŸ’° Value Assessment</Text>
