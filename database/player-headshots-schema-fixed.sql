-- Player Headshots Database Schema Extension (FIXED FOR ACTUAL DB STRUCTURE)
...
-- 2. Create indexes on existing columns for fast lookups
...
-- 3. Create player_headshots table for tracking headshot management
...
-- 4. Create indexes for headshot queries
...
-- 5. Create view for players with headshots (using actual column names)
...
SELECT 
    ...
    COALESCE(ph.headshot_url, p.headshot_url) as headshot_url,
    ph.thumbnail_url,
    ph.source as headshot_source,
    COALESCE(ph.last_updated, p.updated_at) as last_updated,
    CASE WHEN COALESCE(ph.headshot_url, p.headshot_url) IS NOT NULL THEN TRUE ELSE FALSE END as has_headshot
FROM players p
LEFT JOIN player_headshots ph ON p.id = ph.player_id AND ph.is_active = true
WHERE p.active = true
ORDER BY p.sport, p.team, p.name;

-- 6. Function to get player with headshot by name and team
...
-- 7. Function to update player headshot
...
BEGIN
    ...
    SET 
        ...
        external_player_id = COALESCE(external_player_id, p_sportsdata_id::VARCHAR),
        updated_at = now()
...
-- 8. Function to get players missing headshots
...
-- 9. Create trigger to update players.updated_at when headshot changes
...
-- 10. Function to clean up old/inactive headshots
...
-- 11. Create materialized view for headshot statistics (FIXED)
...
-- 12. Function to refresh headshot stats
