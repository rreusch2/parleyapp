...
router.get('/picks', async (req, res) => {
  try {
    ...
    if (isNewUser && userTier === 'free') {
      // For new free users, get picks from the new main.py system (same as Pro users)
      logger.info(`🎁 New free user - checking for system predictions to show as welcome bonus`);
      
      // Get the latest picks from the new system (generated by main.py)
      let query = supabase
        .from('ai_predictions')
        .select('*')
        .not('user_id', 'eq', '00000000-0000-0000-0000-000000000000') // Exclude old system predictions
    ...
    // For free users, get system-wide predictions; for pro users, get all predictions
    if (userTier === 'free') {
      // Free users get the latest system predictions (not user-specific)
      query = query.not('user_id', 'eq', '00000000-0000-0000-0000-000000000000'); // Exclude old system predictions
    } else {
      // Pro users can see all predictions (could be user-specific or system-wide in the future)
      query = query.not('user_id', 'eq', '00000000-0000-0000-0000-000000000000'); // Exclude old system predictions
    }
